...
WITH
  ...
  -- ,
  -- parent_transaction AS ( -- Making the parent_transaction logic its own CTE because this transcends all specific transaction types
,
  elegible AS (
    SELECT DISTINCT
      ...
      transaction_id_ns,
      CASE
        WHEN record_type = 'salesorder' THEN TRUE
        WHEN record_type IN ('cashsale', 'invoice')
        AND createdfrom IS NULL THEN TRUE
        ELSE FALSE
      END elegible
    FROM
      first_pass
    ...
    WHERE
      first_pass.order_id_edw in( 'PB-ST63168/SM','113-7256776-6975450')
  )
SELECT
  first_pass.order_id_edw,
  CONCAT(
