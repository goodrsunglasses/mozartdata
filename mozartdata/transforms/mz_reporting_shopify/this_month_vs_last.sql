...
dates AS  (
   SELECT 
      datediff(day, the_date, (SELECT MAX(processed_timestamp) FROM mz_reporting_shopify.orders) :: date),
      *
   FROM date_map
   WHERE datediff(day, the_date, (SELECT MAX(processed_timestamp) FROM mz_reporting_shopify.orders) :: date) BETWEEN 0 AND 84
),
...
filtered AS (
  SELECT 
    d.the_date,
    CEIL((((SELECT MAX(processed_timestamp) FROM mz_reporting_shopify.orders) :: date) - d.the_date + 1)/28) AS months_ago,
    MOD((((SELECT MAX(processed_timestamp) FROM mz_reporting_shopify.orders) :: date) - d.the_date), 28) AS day_in_month,
    so.*
