...
SELECT
  ...
  transtatus.fullname AS full_status,
  tranline.item,
  case when tranline.itemtype = 'TaxItem' then 'Tax' 
  when tranline.itemtype = 'ShipItem' then 'Ship' 
  end as plan_name,
  null as quantity,
  tranline.rate,
  tranline.netamount,
  null as estgrossprofit,
  null as costestimate,
  CONVERT_TIMEZONE('America/Los_Angeles', tran.createddate) AS timestamp_transaction_pst,
  MD5(CONCAT(order_id_edw, tran.id, item)) AS detail_id
FROM
  netsuite.transaction tran
...
WHERE
  tranline.itemtype IN ('TaxItem', 'ShipItem')
ORDER BY
  order_id_edw,
  recordtype asc