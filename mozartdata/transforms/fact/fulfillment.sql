...
SELECT fulfill.FULFILLMENT_ID_EDW,
	   ...
	   COALESCE(stord.customer_name, ss.customer_name)                        AS customer_name,
	   ...
	   COALESCE(stord.shipdate, ss.shipdate)                                  AS ship_date,
	   COALESCE(stord.country, ss.country)                                    AS country,
	   COALESCE(stord.state, ss.state)                                        AS state,
	   COALESCE(stord.city, ss.city)                                          AS city,
	   COALESCE(stord.postal_code, ss.postal_code)                            AS postal_code,
	   COALESCE(stord.addr_line_1, ss.addr_line_1)                            AS addr_line_1,
	   COALESCE(stord.addr_line_2, ss.addr_line_2)                            AS addr_line_2,
