WITH
  ...
  collection_orders AS (
    ...
    FROM
      fact.order_item oi
      INNER JOIN fact.orders o ON oi.order_id_edw = o.order_id_edw
      LEFT JOIN dim.product p ON p.item_id_ns = oi.item_id_ns
    WHERE
      o.channel = 'Goodr.com'
      AND p.family = 'LICENSING'
  ...
  total_sales AS (
    ...
    FROM
      ...
      INNER JOIN fact.orders o ON co.order_id_edw = o.order_id_edw
    WHERE
