...
dates as (
  SELECT datediff(day, the_date, (SELECT MAX(processed_timestamp) from mz_reporting_specialty_shopify.orders) :: date),
        *
FROM 
  date_map
WHERE 
  datediff(day, the_date, (SELECT MAX(processed_timestamp) from mz_reporting_specialty_shopify.orders) :: date) BETWEEN 0 AND 13
),
...
filtered AS (
  SELECT d.the_date, 
        CEIL((((SELECT MAX(processed_timestamp) FROM mz_reporting_specialty_shopify.orders) :: date) - d.the_date + 1)/7) AS weeks_ago,
        MOD((((SELECT MAX(processed_timestamp) FROM mz_reporting_specialty_shopify.orders) :: date) - d.the_date), 7) AS day_in_week,
        so.* 
  FROM 
    dates AS d
